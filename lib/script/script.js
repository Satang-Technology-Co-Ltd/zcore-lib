...
Script.fromASM = function(str) {
  ...
  while (i < tokens.length) {
    ...
    if (_.isUndefined(opcodenum)) {
      ...
      let isContractByteCode = false;
      if (i === tokens.length - 2 &&
        Opcode(tokens[tokens.length - 1]).toNumber() === Opcode.OP_CREATE) {
          isContractByteCode = true;
      }
      script.chunks.push({
        buf: buf,
        len: buf.length,
        opcodenum: isContractByteCode ? Opcode.OP_PUSHDATA2 : buf.length
      });
      i = i + 1;
    } else if (opcodenum === Opcode.OP_PUSHDATA1 ||

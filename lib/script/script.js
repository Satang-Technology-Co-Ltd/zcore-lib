...
Script.fromASM = function (str) {
  ...
  var i = 0;
  var lastOp = tokens[tokens.length - 1];

  if (lastOp === "OP_CALL" || lastOp === "OP_CREATE") {
    for (var index = 0; index < tokens.length; index++) {
      var data = new Buffer.from(tokens[index], 'hex');
      var opcodenum = data.length;
      if (index === 5 && lastOp === "OP_CALL") {
        script.chunks.push({
          opcodenum: Opcode.OP_CALL
        });
        break;
      } 
      if (index === 4 && lastOp === "OP_CREATE") {
        script.chunks.push({

...
Script.fromASM = function (str) {
  ...
  if ((tokens[tokens.length - 1]) === "OP_CALL" && tokens.length === 6) {
    var version = new Buffer.from(tokens[0], 'hex');
    script.chunks.push({
      buf: version,
      len: version.length,
      opcodenum: version.length
    });
    var gasLimit = new Buffer.from(tokens[1], 'hex');
    script.chunks.push({
      buf: gasLimit,
      len: gasLimit.length,
      opcodenum: gasLimit.length
    });
    var gasPrice = new Buffer.from(tokens[2], 'hex');
    script.chunks.push({
      buf: gasPrice,
      len: gasPrice.length,
    ...
    script.chunks.push({
      ...
      opcodenum: contractAddress.length
    });
    script.chunks.push({
      opcodenum: Opcode.OP_CALL
    });
  } else if ((tokens[tokens.length - 1]) === "OP_CREATE" && tokens.length === 5) {
    var version = new Buffer.from(tokens[0], 'hex');
    script.chunks.push({
      buf: version,
      len: version.length,
